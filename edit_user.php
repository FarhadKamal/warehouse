...
//Update User basic info
  if(isset($_POST['update'])) {
    $req_fields = array('name','username','level','company-name');
    ...
    if(empty($errors)){
       ...
       $com_id   = remove_junk($db->escape($_POST['company-name']));
          $level = (int)$db->escape($_POST['level']);
       $status   = remove_junk($db->escape($_POST['status']));
            $sql = "UPDATE users SET name ='{$name}', username ='{$username}',user_level='{$level}',status='{$status}',designation='{$designation}',com='{$com_id}' WHERE id='{$db->escape($id)}'";
         $result = $db->query($sql);
              ...
              <label for="company">User Role</label>
                <select class="form-control" name="company-name">
                  <?php foreach ($coms as $com ):?>
                   <option <?php if($com['com_id'] === $e_user['com']) echo 'selected="selected"';?> value="<?php echo $com['com_id'];?>"><?php echo ucwords($com['com_name']);?></option>
                <?php endforeach;?>
                </select>
            </div>
            <div class="form-group">
