...
if (isset($_POST['submit'])) {
  ...
  if (empty($errors)) {
    ...
    $results      = find_stock_ledger_by_dates($start_date, $end_date, $location, $product,$com_id );
    //$res          = find_stock_by_product($start_date,$end_date,$location,$product  );
    $resop        = find_opening_stock_ledger_by_dates($start_date, $end_date, $location, $product,$com_id);
...
<body>
  <?php if ($results) : ?>
    <input type="button" class="btn btn-sm" onclick="tableToExcel('excel', 'Stock Ledger Report', 'StockLedgerReport.xls')" value="Export to Excel" />
